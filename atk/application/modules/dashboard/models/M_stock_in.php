...
class M_stock_in extends CI_Model {
    ...
    public function feed_barang($q="", $searchBy="nama") {
        ...
        $sql_rujit = "SELECT 
                        br1.nama_barang, br1.qty,br1.qty_try,br1.buffer_stock,br1.batas,
                        IFNULL(SUM(in1.received_qty), 0)+br1.qty_old AS tot_in, 
                        IFNULL(out2.tot_out,0) AS tot_out,
                        IFNULL((IFNULL(SUM(in1.received_qty),0)+br1.qty_old-IFNULL(out2.tot_out,0)),0) AS stock_ready, 
                        br1.satuan".$addCols."
                        FROM tbl_barang br1 
                        LEFT JOIN tbl_stock_in_item in1 ON br1.barang_id=in1.barang_id 
                        LEFT JOIN tbl_stock_in in2 ON in1.stockin_id=in2.stockin_id 
	...
	$req="";
        ...
        $sqlIn['vals'] = array('@stockin_id', $po, $datepo, $do, $dt, $req);
