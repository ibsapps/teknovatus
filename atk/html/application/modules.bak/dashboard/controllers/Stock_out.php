...
class Stock_out extends CI_Controller {
 ...
 public function penerimaan() {
	...
	$this->load->view('content/penerimaan', $data);
    ...
    public function reject() {
        $this->filterInput()->rejectPost()->setJsonResponse()->sendEmail()->displayView();
    }
    ...
    private function filterPostClosing() {
        ...
        $this->result = array('status'=>1, 'desc'=>'Closing', 'param'=>array('field'=>$field, 'cond'=>$cond));
...
//         $arrColor = array('collect' => 'btn-info', 'approve' => 'btn-success', 'deliver' => 'btn-warning', 'reject' => 'btn-danger');
        foreach ($this->feed['data'] as $key => $feed) {
            $this->feed['data'][$key]['stock_out_qty'] = $this->addFormClosing_qty($feed['stock_out_qty']);
    ...
    private function addFormClosing_id($stotem_id, $satuan) {
        $stotem_form = "<input type=\"hidden\" name=\"stotem_id[]\" value=\"" . $stotem_id . "\" /> <div class=\"btn-group btn-group-toggle\" data-toggle=\"buttons\"><label class=\"btn btn-xs btn-primary\"><input type=\"radio\" name=\"act_todo_".$stotem_id."[]\" value=\"1\" class=\"list_".$stotem_id."\" autocomplete=\"off\"> Deliver</label><label class=\"btn btn-xs btn-warning\"><input type=\"radio\" value=\"2\" name=\"act_todo_".$stotem_id."[]\" class=\"list_".$stotem_id."\" autocomplete=\"off\"> Reject</label></div>";
        return $stotem_form;
    }
